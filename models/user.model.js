...
const UserSchema = new Schema({
    ...
    password: { type: String, required: true, max: 40, min: 8, select: false },
    avgRating: { type: Number, default: -1, min: -1, max: 10},
...
UserSchema.statics.login = function(username, password) {
    return this.findOne({username: username.toLowerCase()}, '+password').exec().then(function(user) {
        if (user) {
